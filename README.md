...
Não há uma suíte automatizada no repositório. A validação deve incluir, em um ambiente de testes:

- conexão com todos os bancos configurados;
- preparação das tabelas em uma cópia descartável;
- processamento de amostras nacionais e internacionais;
- interrupção e retomada do processamento;
- conferência dos contadores por palavra e classe;
- geração e validação do JSON;
- consulta pela tela principal.
